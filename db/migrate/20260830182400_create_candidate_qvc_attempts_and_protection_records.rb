...
class CreateCandidateQvcAttemptsAndProtectionRecords < ActiveRecord::Migration[8.1]
  QVC_OUTCOME_FIELDS_CONSTRAINT = <<~SQL.squish.freeze
    (
      (outcome_code IS NULL AND no_show = FALSE AND outcome_recorded_at IS NULL AND outcome_recorded_by_id IS NULL) OR
      ((outcome_code IS NOT NULL OR no_show = TRUE) AND outcome_recorded_at IS NOT NULL AND outcome_recorded_by_id IS NOT NULL)
    )
  SQL
  PROTECTION_APPEARANCE_FIELDS_CONSTRAINT = <<~SQL.squish.freeze
    (
      (appeared_on IS NULL AND appeared_recorded_at IS NULL AND appeared_recorded_by_id IS NULL) OR
      (appeared_on IS NOT NULL AND appeared_recorded_at IS NOT NULL AND appeared_recorded_by_id IS NOT NULL)
    )
  SQL
  PROTECTION_READY_FIELDS_CONSTRAINT = <<~SQL.squish.freeze
    (
      (protected_on IS NULL AND ready_to_fly_at IS NULL AND ready_recorded_by_id IS NULL) OR
      (protected_on IS NOT NULL AND ready_to_fly_at IS NOT NULL AND ready_recorded_by_id IS NOT NULL)
    )
  SQL

  def change
    create_candidate_qvc_attempts!
    create_candidate_protection_records!
  end

  private

  # rubocop:disable Metrics/MethodLength
  def create_candidate_qvc_attempts!
    ...
    add_qvc_attempt_indexes!
    add_qvc_attempt_constraints!
  end

  def add_qvc_attempt_indexes!
    add_index :candidate_qvc_attempts, :public_id, unique: true
    add_index :candidate_qvc_attempts, %i[candidate_assignment_id attempt_number],
              unique: true,
  ...
  def add_qvc_attempt_constraints!
    ...
    add_check_constraint :candidate_qvc_attempts,
                         QVC_OUTCOME_FIELDS_CONSTRAINT,
                         name: 'candidate_qvc_attempts_outcome_fields_consistent'
  end

  def create_candidate_protection_records!
    ...
    add_protection_record_indexes!
    add_protection_record_constraints!
  end

  def add_protection_record_indexes!
