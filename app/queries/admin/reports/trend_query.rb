...
module Admin
  module Reports
    ...
    class TrendQuery < ApplicationQuery
      ...
      def initialize(scope: Candidate.all, granularity: 'monthly', from: nil, to: nil)
        super()
      ...
      def grouped_counts
        stage_history_scope.group(Arel.sql("date_trunc('#{DATE_TRUNC_UNIT.fetch(@granularity)}', occurred_at)")).count
      end
