    ...
    module Imports
      class BatchExecutionService < ApplicationService
        ...
        def update_from_result!(row_result, result)
          outcome = result.to_h
          return row_result.update!(status: 'committed', error_field: nil, error_code: nil) if outcome[:successful_rows].positive?

          error = outcome.fetch(:errors).first
          update_row_result!(row_result, status: 'skipped', error: { field: error.fetch(:field), code: error.fetch(:code) })
        end
        ...
        def complete!(batch)
          CandidateImportBatch.transaction do
            ...
            batch.update!(status: rejected_rows.zero? && skipped_rows.zero? ? 'completed' : 'partial',
            ...
            self.class.send(:create_audit_event, batch:, request_id: @request_id, action_code: 'candidate_import_committed')
          end
        end
