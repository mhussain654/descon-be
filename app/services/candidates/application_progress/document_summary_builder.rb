...
module Candidates
  module ApplicationProgress
    class DocumentSummaryBuilder < ApplicationService
      ...
      def initialize(assignment:, current_documents_by_type:, requirements:)
      ...
      def blocking_requirement_for(requirement)
        ...
        return blocking_requirement(requirement, 'missing') if document.blank?
        return blocking_requirement(requirement, 'rejected') if document.api_status == 'rejected'
        return unless expired_pcc_requirement?(requirement:, document:)

        blocking_requirement(requirement, 'expired')
      end

      def blocking_requirement(requirement, reason)
